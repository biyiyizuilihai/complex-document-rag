...
from complex_document_rag.ingestion.image_records import resolve_source_image_path
...
def index_single_image(img_id: str, desc_data: dict,
    ...
    """
    node = TextNode(
        # `text` 是核心索引内容，embedding 就是基于这个字段生成的。
        text=desc_data["detailed_description"],

        # `metadata` 不参与 embedding 计算，但会跟随检索结果一起返回。
        # 这些字段主要用于定位原图、预览摘要、做元数据过滤，以及区分结果类型。
        metadata={
            "image_id": img_id,
            ...
            "detailed_description": desc_data.get("detailed_description", ""),
            "display_title": desc_data.get("display_title", "") or desc_data.get("summary", ""),
            "section_title": desc_data.get("section_title", ""),
