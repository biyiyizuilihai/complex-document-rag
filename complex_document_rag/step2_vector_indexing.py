...
from complex_document_rag.pipeline_utils import list_document_files, project_root_from_file
...
def build_text_index(docs_dir: str) -> VectorStoreIndex:
    ...
    # 它内置了对 PDF、Markdown、纯文本等格式的解析器
    documents = SimpleDirectoryReader(docs_dir).load_data()
    print(f"加载了 {len(documents)} 个文档")

    # StorageContext 告诉 LlamaIndex 使用 Qdrant 而不是默认的内存存储
    storage_context = StorageContext.from_defaults(
        vector_store=text_store
    )

    # VectorStoreIndex.from_documents 是一站式方法：
    #   自动完成 分块 → embedding → 存储 的全部流程
...
def build_text_index_from_documents(documents: list) -> VectorStoreIndex:
    ...
    --------------------------------------------------------
    该接口供 step0_document_ingestion.py 直接复用，避免必须先落盘到 docs/ 目录。
    """
