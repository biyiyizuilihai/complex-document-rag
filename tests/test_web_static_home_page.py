...
class WebStaticHomePageTestCase(unittest.TestCase):
    ...
    def test_markdown_table_renderer_ignores_logical_table_comments_and_prefers_display_titles(self):
        html = self._load_html()

        self.assertIn("function stripLogicalTableComments", html)
        self.assertIn("logical-table-page", html)
        self.assertIn("item.display_title || item.caption || item.summary", html)
    ...
    def test_mermaid_blocks_are_supported_in_answers(self):
        ...
        self.assertIn("mermaid-card", html)
        self.assertIn("mermaidModal", html)
        self.assertIn("openMermaidModal", html)
        self.assertIn("放大查看", html)
        self.assertIn("mermaidZoomIn", html)
        self.assertIn("mermaidZoomOut", html)
        self.assertIn("mermaidResetView", html)
        self.assertIn("mermaidFitToScreen", html)
        self.assertIn("fitMermaidToScreen()", html)
        self.assertIn("updateMermaidModalTransform()", html)
        self.assertIn("mermaidModalViewport", html)

    def test_streaming_mermaid_waits_for_closed_fence_before_render(self):
        html = self._load_html()

        self.assertIn("function hasClosedMermaidFence", html)
        self.assertIn("if (eventName === \"chunk\")", html)
        self.assertIn("const canRenderMermaid = hasClosedMermaidFence(streamedAnswer);", html)
        self.assertIn("if (canRenderMermaid)", html)
        self.assertIn("else {", html)
        self.assertIn("answerBox.innerHTML = renderAnswerMarkdown(streamedAnswer, { deferMermaid: true });", html)
    ...
    def test_archived_turns_preserve_assets_and_evidence_markup(self):
        html = self._load_html()

        self.assertIn("answerAssetsHtml: answerAssets.innerHTML", html)
        self.assertIn("answerSourcesHtml: answerSources.innerHTML", html)
        self.assertIn("textResultsHtml: textResults.innerHTML", html)
        self.assertIn("imageResultsHtml: imageResults.innerHTML", html)
        self.assertIn("tableResultsHtml: tableResults.innerHTML", html)
        self.assertIn("turn.answerAssetsHtml", html)
        self.assertIn("turn.answerSourcesHtml", html)
        self.assertIn("turn.tableResultsHtml", html)

    def test_submitted_query_is_cleared_immediately_after_send(self):
        html = self._load_html()

        self.assertIn("function clearQueryInputAfterSubmit()", html)
        self.assertIn("clearQueryInputAfterSubmit();", html)
