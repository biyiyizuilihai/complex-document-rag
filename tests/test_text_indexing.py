...
class TextIndexingTests(unittest.TestCase):
    def test_build_text_index_from_documents_uses_vectorstore_pipeline(self):
        ...
        with patch("complex_document_rag.step2_vector_indexing.StorageContext.from_defaults") as mock_storage_context:
            mock_storage_context.return_value = "storage-context"
            with patch("complex_document_rag.step2_vector_indexing.VectorStoreIndex.from_documents") as mock_from_documents:
